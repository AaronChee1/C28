...
let waterSound, backgroundMusic, cannonExplosion, pirateLaughSound;
  ...
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      if (boats[i]) {
        ...
        boats[i].display();
        boats[i].animate();
        let collision = Matter.SAT.collides(tower, boats[i].body)
        if(collision.collided && !boats[i].isBroken){
          //piratelaughsound is not playing and islaughing is false
